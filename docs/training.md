...
accelerate launch \
    ...
    --output_dir exp/bantuvoice_emilia
```
...
```json
{
    "resume_from_checkpoint": "exp/bantuvoice/checkpoint-100000"
}
```

## Initializing from a Pretrained Model

To start training from a pretrained BantuVoice checkpoint (for fine-tuning):

```json
{
    "init_from_checkpoint": "exp/bantuvoice/checkpoint-100000"
}
```
...
```bash
tensorboard --logdir exp/bantuvoice_emilia/tensorboard
```
