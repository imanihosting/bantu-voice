...
function FileDropzone({
  ...
  async function trimAudioFile(inputFile: File): Promise<File> {
    ...
    const trimmedLength = Math.floor(MAX_DURATION_S * decoded.sampleRate);
    ...
    const trimmedName = inputFile.name.replace(/\.[^.]+$/, "") + "_trimmed.wav";

    toast.info(`Audio trimmed to ${MAX_DURATION_S}s for voice cloning`, {
      description: `Original: ${Math.round(decoded.duration)}s → ${MAX_DURATION_S}s`,
    });
  ...
  const {
    ...
    onDrop: async (acceptedFiles) => {
      if (acceptedFiles.length > 0) {
        setIsTrimming(true);
        try {
          const trimmed = await trimAudioFile(acceptedFiles[0]);
          onFileChange(trimmed);
        } catch {
  ...
  if (isTrimming) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border px-6 py-10">
        <div className="size-6 animate-spin rounded-full border-2 border-primary border-t-transparent" />
        <p className="text-sm text-muted-foreground">Trimming audio to 30s...</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1.5">
        ...
        <p className="text-center text-sm text-muted-foreground">
          Max 20MB · Audio will be trimmed to 30s for voice cloning
        </p>
      </div>
        ...
        <Command>
          ...
          <CommandList>
            ...
            {REGION_ORDER.map((region) => {
              const langs = LANGUAGE_OPTIONS.filter((l) => l.region === region);
