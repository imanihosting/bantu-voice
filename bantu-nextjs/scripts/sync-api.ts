/**
 * Fetches the OpenAPI spec from the BantuVoice TTS API and generates TypeScript types.
 *
 * Usage:
 *   BANTUVOICE_API_URL=https://your-api-url npm run sync-api
 *
 * Or with .env file:
 *   npm run sync-api
 */
...
const OUTPUT_PATH = path.resolve(__dirname, "../src/types/bantuvoice-api.d.ts");

async function main() {
  const apiUrl = process.env.BANTUVOICE_API_URL;

  if (!apiUrl) {
    console.error("Error: BANTUVOICE_API_URL environment variable is required");
    process.exit(1);
  }
