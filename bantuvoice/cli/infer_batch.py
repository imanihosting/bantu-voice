...
"""Batch inference CLI for BantuVoice.
...
Usage:
    bantuvoice-infer-batch --model k2-fsa/OmniVoice \
...
from bantuvoice.models.bantuvoice import BantuVoice
import soundfile as sf

from bantuvoice.utils.audio import load_audio
from bantuvoice.utils.common import str2bool
from bantuvoice.utils.data_utils import read_test_list
...
def get_parser():
    parser = argparse.ArgumentParser(description="Infer BantuVoice Model")
    parser.add_argument(
        "--model",
        type=str,
...
def process_init(rank_queue, model_checkpoint, warmup=0):
    ...
    via ``BantuVoice.from_pretrained()``.
    """
    global worker_model
    ...
    worker_model = BantuVoice.from_pretrained(
        model_checkpoint,
