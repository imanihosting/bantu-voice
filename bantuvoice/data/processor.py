...
"""Training sample processor for BantuVoice.
...
Used by ``bantuvoice.training.builder`` to build the data pipeline.

Contains two processor classes:
- ``BantuVoiceSampleProcessor``: Full processor used for training.
- ``BantuVoiceSimpleSampleProcessor``: Simplified processor (not used for training).
"""
...
class BantuVoiceSampleProcessor:
    """
...
class BantuVoiceSimpleSampleProcessor:
    """
    Handles the logic of processing a raw sample into tensors
    (masking, tokenization, etc.).
    This is a simpler version that does not include language, instructions,
        or denoising prompts.
    We do not use it for training as BantuVoiceSampleProcessor can cover this case.
