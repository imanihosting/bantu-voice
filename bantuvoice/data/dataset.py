...
loading. Used by ``bantuvoice.training.builder.build_dataloaders()`` to construct
...
- ``JsonlDatasetReader``: Reads audio/text pairs from a JSONL manifest file.
    Used by data processing scripts (e.g. ``bantuvoice/scripts/``).
...
from bantuvoice.utils.audio import load_audio, load_audio_bytes
