...
from a ``TrainingConfig``. Called by ``bantuvoice.cli.train`` to set up training.
...
from bantuvoice.data.batching import PackingIterableDataset, StreamLengthGroupDataset
from bantuvoice.data.collator import PackingDataCollator, PaddingDataCollator
from bantuvoice.data.dataset import WebDatasetReader, prepare_data_manifests_from_json
from bantuvoice.data.processor import BantuVoiceSampleProcessor
from bantuvoice.models.bantuvoice import BantuVoice, BantuVoiceConfig, _resolve_model_path
from bantuvoice.training.config import TrainingConfig
...
def build_model_and_tokenizer(
...
) -> Tuple[BantuVoice, AutoTokenizer]:
    ...
    if config.init_from_checkpoint:
        ...
        model = BantuVoice(config=ov_config, llm=llm)
...
def build_dataloaders(
    ...
    processor = BantuVoiceSampleProcessor(
