...
"""Core BantuVoice model implementation.

Defines the ``BantuVoice`` model class, generation config, and inference pipeline.
This is the main entry point for both inference and training:

- **Inference**: ``BantuVoice.from_pretrained()`` loads the model, then
...
- **Training**: ``model.forward()`` computes the training loss; the model is
  built and used by ``bantuvoice.training.builder`` and ``bantuvoice.training.trainer``.
...
from bantuvoice.utils.audio import (
    cross_fade_chunks,
    fade_and_pad_audio,
    load_audio,
    remove_silence,
...
from bantuvoice.utils.duration import RuleDurationEstimator
from bantuvoice.utils.lang_map import LANG_IDS, LANG_NAMES
from bantuvoice.utils.text import add_punctuation, chunk_text_punctuation
from bantuvoice.utils.voice_design import (
    _INSTRUCT_ALL_VALID,
    _INSTRUCT_EN_TO_ZH,
...
@dataclass
class BantuVoiceGenerationConfig:
    num_step: int = 32
...
class GenerationTask:
    ...
    def get_indices(self, config: BantuVoiceGenerationConfig, frame_rate: int):
...
@dataclass
class BantuVoiceModelOutput(ModelOutput):
...
class BantuVoiceConfig(PretrainedConfig):
    model_type = "bantuvoice"
...
class BantuVoice(PreTrainedModel):
    _supports_flex_attn = True
    _supports_flash_attn_2 = True
    _supports_sdpa = True
    config_class = BantuVoiceConfig

    def __init__(self, config: BantuVoiceConfig, llm: Optional[PreTrainedModel] = None):
    ...
    def generate(
        ...
        generation_config: Optional[BantuVoiceGenerationConfig] = None,
        **kwargs,
    ) -> list[np.ndarray]:
        ...
        if self.audio_tokenizer is None or self.text_tokenizer is None:
            raise RuntimeError(
                ...
                "loaded the model with BantuVoice.from_pretrained()."
            )
        gen_config = (
            generation_config
            if generation_config is not None
            else BantuVoiceGenerationConfig.from_dict(kwargs)
        )
    ...
    def create_voice_clone_prompt(
        ...
        if self.audio_tokenizer is None:
            raise RuntimeError(
                ...
                "with BantuVoice.from_pretrained()."
            )
    ...
    def _generate_chunked(
        self, task: GenerationTask, gen_config: BantuVoiceGenerationConfig
    ...
    def _generate_iterative(
        ...
        Args:
            ...
            gen_config: A :class:`BantuVoiceGenerationConfig` controlling
...
def _resolve_language(language: Optional[str]) -> Union[str, None]:
    from bantuvoice.utils.lang_map import LANG_IDS, LANG_NAME_TO_ID
...
AutoConfig.register("bantuvoice", BantuVoiceConfig)
AutoModel.register(BantuVoiceConfig, BantuVoice)
